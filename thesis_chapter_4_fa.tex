...
% Prevent AddToHook errors in hyperref/bidi
...
% Extra bidi tabular fix - bypass broken bidi tabular init
\AtBeginDocument{
  \@ifpackageloaded{bidi}{
    \def\bidi@tabular@init{\ar@ialign}
  }{}
}
\makeatother
\ExplSyntaxOff
...
\ifdefined\setlatinfont
  \setlatinfont[Scale=1.0]{Times New Roman}
\else
  \ifdefined\setlatintextfont
...
پنج حوضه از شش حوضه در \textbf{سال ۲۰۲۲} — آخرین سال دوره مطالعه — به بدترین شرایط
خشکسالی هیدرولوژیکی رسیدند. فروپاشی همزمان در تمامی حوضه‌ها در ۲۰۲۱–۲۰۲۲ یک
\textbf{رویداد خشکسالی هیدرولوژیکی در مقیاس ملی} را تشکیل می‌دهد.
...
\lr{SPI} افزایش می‌یابند. برای مرکزی، $r$ از $0.117$ (\lr{SPI-3}، بدون معنی‌داری) به
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    \textbf{حوضه} &
    \textbf{دقت حالت خشکسالی (\%)} &
    \textbf{MAE (سانتی‌متر)} &
    \textbf{RMSE (سانتی‌متر)} &
    \textbf{MAE (DSI)} &
    \textbf{RMSE (DSI)} &
    \textbf{نتیجه} \\
    \midrule
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    \toprule
    \textbf{محدودیت} & \textbf{شدت} & \textbf{اقدام کاهشی اعمال‌شده} & \textbf{اقدام پیشنهادی آینده} \\
    \midrule
    فقدان آب زیرزمینی درجا  & بالا     & معیارسنجی با مقالات، پروکسی \lr{GWS} &
                               یکپارچه‌سازی \lr{GLDAS}؛ توافق داده چاه \\
    فقدان داده رواناب/جریان & متوسط    & مانده بیلان آبی &
                               سوابق ایستگاه هیدرومتری \\
    تفکیک خشن \lr{GRACE}    & متوسط    & فیلتر \lr{DDK}، مستندسازی نشت &
                               مقیاس‌سازی پایین \lr{GRACE} \\
    شکاف مأموریت ۲۰۱۷–۲۰۱۸ & کم تا متوسط & درون‌یابی خطی + علامت‌گذاری &
                               روش‌های پرکردن شکاف با آنسامبل \\
    فقدان مشاهدات مستقیم $ET$ & متوسط  & پروکسی مانده \lr{WB} (سهم ۸۰\%) &
